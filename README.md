...
상단 **ChatGPT / Gemini / LM Studio** 버튼이 제공자 선택과 연결을 함께 처리합니다. 왼쪽 설정에서는 선택한 제공자의 모델만 고르면 됩니다.

- **청록색 · 사용 중:** 현재 생성에 사용할 제공자입니다.
- **초록색 · 연결됨 · 선택:** 연결되어 있으며 누르면 바로 전환됩니다.
- **회색 · 로그인/키 입력/자동 연결:** 아직 연결되지 않았습니다. 누르면 필요한 연결 절차를 시작합니다.
- **주황색 · 선택됨/연결 중/로그인 대기:** 제공자는 선택했지만 준비가 필요합니다.

선택한 제공자와 각 모델은 다음 실행에도 기억합니다. 생성하는 동안 제공자·모델 전환은 잠겨 결과가 섞이지 않습니다.

### ChatGPT

OpenAI API Key는 필요 없습니다.

1. 상단 `ChatGPT` 버튼을 누릅니다.
2. 로그인 창이 뜨면 OpenAI/Codex 로그인을 끝냅니다.
3. 앱으로 돌아오면 연결 상태가 자동 갱신됩니다. 버튼을 다시 누를 필요가 없습니다.
4. `사용 중` 표시가 나오면 준비 완료입니다.
...
1. 상단 `Gemini` 버튼을 누릅니다. 저장된 키가 있으면 바로 전환됩니다.
2. Gemini API Key를 붙여넣습니다.
3. `저장하고 사용` 또는 Enter를 누릅니다.
4. `사용 중` 표시가 나오면 준비 완료입니다. 키를 바꾸거나 삭제할 때는 `API 키 관리`를 누릅니다.
...
## 5. LM Studio 로컬 이미지 분석

LM Studio 0.4 이상과 아래 모델 및 비전 보조 파일(`mmproj`)을 먼저 설치합니다.

| 선택지 | 용도 |
|---|---|
| Gemma 4 E4B · Q5_K_M | 적은 메모리로 실행 |
| Gemma 4 E4B · Q8_0 | 양자화 정밀도 우선 |

1. 상단 `LM Studio`를 누르면 로컬 서버를 자동으로 연결합니다. 서버가 꺼져 있으면 설치된 `lms` CLI로 시작합니다.
2. 모델 목록에서 Q5_K_M 또는 Q8_0을 선택합니다. 처음에는 Q5_K_M을 선택하며, 한 모델만 사용 가능하면 그 모델을 자동 선택합니다.
3. 이미지를 넣고 생성합니다. 모델은 첫 요청에 자동으로 로드되므로 LM Studio에서 따로 불러올 필요가 없습니다.

실제 설치 목록에서 모델·양자화·비전 지원을 함께 확인합니다. DECKARD 등 다른 모델은 대신 선택하지 않습니다. 비전 보조 파일이 없으면 `설치 확인 필요`로 표시합니다. 파일 이름을 바꾸거나 모델을 자동 다운로드하지는 않습니다.

로컬 제공자를 선택하면 이미지는 로컬 LM Studio로만 전송됩니다. 기본 주소는 `http://127.0.0.1:1234`이며 키 입력은 필요 없습니다. 첫 생성은 로딩 때문에 더 오래 걸릴 수 있습니다. 서술형과 Booru 태그형 모두 사용할 수 있습니다.

사용자 지정 포트는 서버 환경변수 `LMSTUDIO_BASE_URL`로 지정할 수 있습니다(로컬 HTTP 주소만 허용). LM Studio에서 API 인증을 켰다면 `LMSTUDIO_API_TOKEN` 환경변수를 설정하세요. 자동 연결이 실패하면 LM Studio를 한 번 실행한 뒤 상단 버튼을 다시 누르면 됩니다.
